-- AXELUS-WAF — Kong Gateway Plugin
-- Integrates AXELUS-WAF detection engine into Kong API Gateway.
-- Publisher: OPTIMIUM NEXUS LLC — https://www.optimiumnexus.com
--
-- Installation:
--   1. Copy this file to /usr/local/share/lua/5.1/kong/plugins/axelus-waf/handler.lua
--   2. Copy schema.lua to the same directory
--   3. Add "axelus-waf" to Kong's plugins list in kong.conf
...
function AXELUSHandler:access(conf)
  local ngx    = ngx
  local req    = ngx.req
  ...
  if conf.geoip_enabled then
    local geo_result = self:call_axelus(conf, "/api/open/geoip/evaluate", { ip = client_ip })
    ...
    if geo_result and geo_result.lookup then
      ngx.req.set_header("X-AXELUS-Country",    geo_result.lookup.country_code or "XX")
      ngx.req.set_header("X-AXELUS-Risk-Score", tostring(geo_result.lookup.risk_score or 0))
    end
  end

  -- ── 2. Threat Intel IP check ───────────────────────────────────────────────
  if conf.threat_intel_enabled then
    local ti_result = self:call_axelus(conf, "/api/open/threat-intel/check", { ip = client_ip })
  ...
  if conf.rate_limit_enabled then
    local rl_result = self:call_axelus(conf, "/api/open/ratelimit/check", {
      ip       = client_ip,
      path     = uri,
      method   = method,
  ...
  if conf.dpi_enabled and (method == "POST" or method == "PUT" or method == "PATCH") then
    local dpi_result = self:call_axelus(conf, "/api/open/inspect", payload)
    ...
    if dpi_result then
      ngx.req.set_header("X-AXELUS-Threat-Score", tostring(dpi_result.score or 0))
    end
  end
  ...
  if conf.license_check_header then
    ...
    if lic_key then
      local lic_result = self:call_axelus(conf, "/api/v1/licensing/validate", {
        license_key = lic_key,
        feature     = conf.license_required_feature,
      })
      if not lic_result or not lic_result.valid then
        self:block_status(conf, 403, "license", "Invalid or missing AXELUS license")
        return
      end
      ngx.req.set_header("X-AXELUS-License-Tier", lic_result.tier or "unknown")
    end
  end

  -- ── 6. Honeypot path check ────────────────────────────────────────────────
  if conf.honeypot_enabled then
    local hp_result = self:call_axelus(conf, "/api/open/honeypot/check", { path = uri })
  ...
  -- All checks passed — add AXELUS inspection header
  ngx.req.set_header("X-AXELUS-Inspected", "1")
  ngx.req.set_header("X-AXELUS-Version",   "1.0.0")
end

-- ── HTTP call to AXELUS API ─────────────────────────────────────────────────

function AXELUSHandler:call_axelus(conf, path, body)
  local httpc = http.new()
  httpc:set_timeout(conf.timeout_ms or 50)

  local ok, err = httpc:connect(conf.axelus_host, conf.axelus_port or 9443)
  if not ok then
    kong.log.warn("[axelus] connection failed: ", err)
    return nil
  end
  ...
  if not res then
    kong.log.warn("[axelus] request failed: ", err)
    return nil
  end
...
function AXELUSHandler:block(conf, reason_code, reason_msg, details)
  self:block_status(conf, conf.block_status or 403, reason_code, reason_msg, details)
end

function AXELUSHandler:block_status(conf, status, reason_code, reason_msg, details)
  ngx.header["X-AXELUS-Blocked"]       = "1"
  ngx.header["X-AXELUS-Block-Reason"]  = reason_code
  ngx.header["X-AXELUS-Request-Id"]    = ngx.var.request_id or ""
  ...
  kong.log.warn(string.format(
    "[axelus] BLOCKED ip=%s path=%s reason=%s",
