#!/usr/bin/env bash
# IronWall — Automated Backup Script
# Backs up PostgreSQL database and configuration files.
# Usage: bash scripts/backup.sh [--dest /path/to/backups]
set -euo pipefail

BACKUP_DIR="${IRONWALL_BACKUP_DIR:-/data/ironwall-backups}"
SAFELINE_DIR="${SAFELINE_DIR:-/data/ironwall}"
RETAIN_DAYS="${BACKUP_RETAIN_DAYS:-30}"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
BACKUP_PATH="$BACKUP_DIR/ironwall-$TIMESTAMP"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        --dest) BACKUP_PATH="$2/ironwall-$TIMESTAMP"; mkdir -p "$BACKUP_PATH"; shift 2 ;;
        *) shift ;;
    esac
done

log "Starting IronWall backup → $BACKUP_PATH"
...
# ── 5. Compress backup ────────────────────────────────────────────────────────
ARCHIVE="$BACKUP_DIR/ironwall-$TIMESTAMP.tar.gz"
tar -czf "$ARCHIVE" -C "$BACKUP_DIR" "ironwall-$TIMESTAMP"
rm -rf "$BACKUP_PATH"
log "Backup archive: $ARCHIVE ($(du -sh "$ARCHIVE" | cut -f1))"

# ── 6. Prune old backups ──────────────────────────────────────────────────────
log "Pruning backups older than ${RETAIN_DAYS} days..."
find "$BACKUP_DIR" -name "ironwall-*.tar.gz" -mtime "+$RETAIN_DAYS" -delete
REMAINING=$(find "$BACKUP_DIR" -name "ironwall-*.tar.gz" | wc -l)
